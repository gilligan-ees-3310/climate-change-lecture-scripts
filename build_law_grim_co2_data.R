...
get_grim <- function(data.file = grim_file, data.dir = 'data', quiet = TRUE) {
  download.file(grim_url, destfile = file.path(data.dir, 'keeling', grim_file), quiet = quiet, mode = 'wb')
}
...
build_smoothed_co2 <- function(update=FALSE, data.dir = 'data', grim_file = grim_file, law_file = law_co2_file) {
  if (update) {
    get_grim(grim_file, data.dir)
    get_law_dome_data(law_file, data.dir)
  }
  co2 <- load_law_dome_co2(law_file, data.dir)
  grim <- read.csv(file.path(data.dir, 'keeling', grim_file), header=T, skip=24, stringsAsFactors = F)
  grim <- na.omit(grim)
  ...
  saveRDS(smoothed_co2, file = file.path(data.dir, 'paleo', 'smoothed_co2.Rds'))
}
